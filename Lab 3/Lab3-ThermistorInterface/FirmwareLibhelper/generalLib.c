...
void ADC10_Init(void){
    // Set up ADC
    // --- Enable internal reference 1.5 V ---
    while (REFCTL0 & REFGENBUSY);        // Wait if ref generator is busy
    REFCTL0 |= REFVSEL_0 | REFON;        // 1.5 V reference ON
    __delay_cycles(400);             // Allow reference to settle      // Wait until reference is ready

    // --- ADC core setup ---
    ADC10CTL0 &= ~ADC10ENC;              // Disable ADC before config
    ADC10CTL0 = ADC10ON | ADC10SHT_3;    // Long SHT (max valid code); ADC ON
    ADC10CTL1 = ADC10SHP | ADC10SSEL_3   // Sampling timer; ADC10CLK=SMCLK
              | ADC10CONSEQ_0;           // Single-channel, single-conversion
    ADC10CTL2 = ADC10RES;                // 10-bit result

    // --- Memory control: channel + reference selection ---
    ADC10MCTL0 = ADC10INCH_0             // A0 input channel
               | ADC10SREF_1;            // Vref+ = internal, Vref- = AVSS
...
unsigned int ADC10_Sample(unsigned char channel){
    // Disable conversions before changing channel
    ADC10CTL0 &= ~ADC10ENC;

    // Configure channel while preserving reference selection
    ADC10MCTL0 = (ADC10MCTL0 & ~ADC10INCH_15) | channel | ADC10SREF_1;
